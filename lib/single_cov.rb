...
module SingleCov
  ...
  class << self
    ...
    def default_tests = glob("{test,spec}/**/*_{test,spec}.rb")

    def glob(pattern) = Dir["#{root}/#{pattern}"].map! { |f| f.sub("#{root}/", '') }

    def indexes(list, find) = list.each_with_index.filter_map { |v, i| i if v == find }
    ...
    def running_single_file? = COVERAGES.size == 1
